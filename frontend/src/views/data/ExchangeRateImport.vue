<script setup>
// 历史汇率导入页：仅负责导入相关功能（上传、下载模板、导入记录、错误明细）
// 汇率的业务操作（查看、编辑、删除）请在「数据管理」页面完成
...
// 页面状态保持（页号切换页面后自动恢复）
const { loadField, saveField } = usePageState('ExchangeRateImport')

// 导入记录表格（查询 import_batch 表，sourceType=EXCHANGE_RATE）
const tableData = ref([])
const total = ref(0)
...
async function loadData() {
  ...
  try {
    const res = await request({
      ...
      method: 'get',
      params: { ...page, sourceType: 'EXCHANGE_RATE' }
    })
...
// 下载汇率导入模板（调用后端接口，含表头 + 红色示例数据行）
async function downloadTemplate() {
  try {
    const res = await axios.get('/api/data/import/template/exchange-rate/download', {
      responseType: 'blob',
      timeout: 30000,
...
async function customUpload({ file }) {
  ...
  try {
    ...
    if (result.failed > 0) {
      // 有失败行：弹出错误明细窗口
      ElMessage.warning(`导入完成：新增 ${result.inserted} 条，更新 ${result.updated} 条，失败 ${result.failed} 条，请查看错误明细`)
      if (result.batchNo) {
...
<template>
  <div class="page-card">
    ...
    >
      <template #title>
        <span>通过 Excel/CSV 文件批量导入历史汇率，免去编写 SQL 脚本。</span>
      </template>
      <div style="font-size: 12px; color: #909399; margin-top: 4px; line-height: 1.6">
        支持表头命名（自动识别）：日期 / 汇率日期 / rate_date、源币种 / 币种 / from_currency、目标币种 / to_currency、汇率 / rate、来源 / source（可选）<br/>
        支持日期格式：yyyy-MM-dd、yyyy/MM/dd、yyyy.MM.dd、yyyyMMdd。同日同币对已存在则覆盖更新，导入后自动刷新内存汇率缓存。
      </div>
    </el-alert>
    ...
    <div class="mb-16" v-loading="uploading">
      ...
      >
        ...
        <div class="el-upload__text">将汇率 Excel/CSV 文件拖到此处，或<em>点击上传</em></div>
        <template #tip>
          <div class="el-upload__tip">支持 .xls / .xlsx / .csv 文件，上传后自动解析并入库</div>
        </template>
      </el-upload>
    </div>
